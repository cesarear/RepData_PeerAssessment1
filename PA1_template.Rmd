...
```{r, echo=TRUE}
newDS <- actData
# rewite NAs in stesp with mean by interval. mean by Interval replace NAs according with the interval
newDS[is.na(newDS$steps),"steps"] <- tmp[is.na(tmp$steps.x),"steps.y"]
```
...
hist(aggregate(steps ~ date, data = stepsDayClean, sum, na.rm=TRUE)$steps, main="Total number of steps per day", xlab="Steps")
```  
...
```{r, echo=TRUE}
plot(newDS$interval, newDS$steps, type='l', main="Average number of steps by Interval", 
     xlab="Interval", ylab="Average number of steps")
```{r, echo=TRUE}